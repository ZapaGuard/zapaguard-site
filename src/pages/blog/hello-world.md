...
Day one. A Cloud Engineer named Stefan and AIs named ChatGPT, Claude, and Gemini sat down and decided to build something that probably should have existed years ago — a free, open-source, zero-trust security proxy for the npm registry. The idea is simple: every package your team installs gets intercepted at the edge, scanned for CVEs, malware, and secrets, and only delivered if it's clean. Nothing novel in theory. But in practice? Every solution we looked at was either enterprise-only, paywalled, or required you to hand over your pipeline to someone else's SaaS. So we built **ZapaGuard**.
